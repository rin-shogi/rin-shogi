...
### 1.1 ハンドル名と trip を決める

floodgate(shogi-server)は **Web フォームによる事前登録は不要**。
公式案内( http://wdoor.c.u-tokyo.ac.jp/shogi/floodgate.html )より:

- 接続先: `wdoor.c.u-tokyo.ac.jp:4081`
- ログイン名(ハンドル): 任意。他のプログラムと重ならないオリジナルなものを
- パスワード(CSA モード): `<game_type>,<trip>` 形式
  - `game_type` = マッチメイキングキューの識別子(時間制を兼ねる)。
    本プロジェクトは **`floodgate-300-10F`**(5 分切れ負け + 10 秒フィッシャー)
  - `trip` = 同名ユーザを区別する秘密文字列(自分で決める)
  - **平文で流れる** ため他用途と兼用しないこと
- 毎時 0 分・30 分近辺に参加プログラム同士でマッチが組まれる
- 15 試合程度でレーティングが計算される

本プロジェクトの設定:

| 項目 | 値 |
|---|---|
| ハンドル | `Rin-suisho5-v1`(命名規則: `Rin-<eval>-v<engine>`、評価関数を差し替えたら別ハンドル) |
| game_type | `floodgate-300-10F`(`configs/match/floodgate_v1.yaml` で指定) |
| trip | 任意の秘密文字列(`.env` の `FLOODGATE_PASSWORD` に設定) |

接続時に送られる LOGIN 行は次のようになる:

```
LOGIN Rin-suisho5-v1 floodgate-300-10F,<your-trip>
```
