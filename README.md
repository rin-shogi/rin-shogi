...
├── .env.example              シークレットテンプレ(.env は .gitignore 済み)
...
├── kifu/                     floodgate で指した対局棋譜(yyyy/mm/dd/<game>.csa、コミット対象)
├── scripts/                  Python オーケストレーション
│   ├── floodgate_client.py   floodgate 接続クライアント(ticket-002)
│   └── utils/                csa.py / csa_usi.py / usi.py / config.py 等
├── configs/                  設定 YAML(コードではなくデータ駆動)
├── tests/                    乾燥テスト・モック(mock_csa_server.py 等)
...
└── docs/                     開発手順書
    ...
    ├── RATING.md             floodgate レーティング推移メモ
...
### 0. シークレット設定(floodgate / HuggingFace を使う場合)
...
| 環境変数 | 用途 |
|---|---|
| `FLOODGATE_PASSWORD` | floodgate(<http://wdoor.c.u-tokyo.ac.jp/shogi/>)登録時のパスワード |
| `HUGGINGFACE_TOKEN` | HuggingFace Hub(<https://huggingface.co/rin-shogi/Rin-evals>)書き込みトークン |

`.env` は `.gitignore` 済み(コミット禁止)。`.env.example` のみリポジトリに含まれる。
スクリプトは起動時に `.env` を自動ロードする(`python-dotenv`)。
...
```powershell
# 1. .env に FLOODGATE_PASSWORD を設定済みの状態で
# 2. floodgate に Rin-suisho5-v1 のハンドルで登録済みの状態で
python scripts/floodgate_client.py --config configs/match/floodgate_v1.yaml
```

事前にローカル乾燥テストで動作を確認できる(実 floodgate に繋がない):

```powershell
python tests/test_floodgate_client_dryrun.py
```

詳細手順・運用ガイドラインは [`docs/FLOODGATE.md`](docs/FLOODGATE.md)、
レーティング推移は [`docs/RATING.md`](docs/RATING.md) 参照。
